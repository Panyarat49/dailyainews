...
The Routine needs very little env — LINE moved out to GitHub Actions (Step 6). The skill reads:
...
`LINE_CHANNEL_ACCESS_TOKEN` / `LINE_TO` are **not read by the skill** — they live in GitHub repo secrets and are consumed by `.github/workflows/line-notify.yml`.
...
Rules:
- Story heading format is fixed: `### N. {Company} ({TICKER} · Tier {n}) — {Headline} — [{Publisher}]({URL})`. The trailing ` — [{Publisher}]({URL})` must be present for single-story slots (the LINE parser depends on it).
...
**Length budget (per story):** ~**80-160 Thai words** — about one short reporting paragraph plus one integrated-perspective paragraph. Roundup sub-items: ~50-90 words each. TL;DR bullets: one line each. Keep the whole brief skimmable on a phone (comfortably under the LINE 5000-char limit).
...
5. If the commit fails: print the full error, do **not** retry, do **not** proceed to LINE.

> Note for operators: commit `sources.md` / `perspectives.md` **in the same commit** as the brief when the connector allows, so the brief is the HEAD of the push (the LINE workflow resolves the notify target from the pushed brief).

---

## Step 6 — LINE push (handled externally)

**This skill does NOT send to LINE.** `WebFetch` in Claude Web Routine accepts only `(url, prompt)` — no method/headers/body — so it cannot POST to `api.line.me` with a Bearer token.

LINE dispatch is handled by `.github/workflows/line-notify.yml`, which triggers on pushes affecting `articles/*-brief.md`, parses the brief (title + TL;DR + per-story headline/link, including roundup sub-items), and POSTs via `curl`.

What the skill does here:
1. Nothing that talks to LINE directly.
2. Print: `LINE: dispatched by .github/workflows/line-notify.yml (see Actions tab for delivery status)`.
3. A NO-OP commit (5a) means no push → the workflow does not fire. Correct.

To force a re-notify: `gh workflow run line-notify.yml -f file=articles/{TODAY}-brief.md`.
...
LINE: dispatched by .github/workflows/line-notify.yml (see Actions tab)
```
...
| Today's article identical | NO-OP. LINE does not fire (intentional). |
| LINE issues | Not this skill's concern — see `.github/workflows/line-notify.yml`. |
