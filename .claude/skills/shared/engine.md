...
`on_allowlist`, `age_h`, `keywords_matched`, **the ranking fields** (`score` +
`score_breakdown`; `cluster_size` = how many outlets ran the story; `source_role` =
primary/citation/screening; and for watchlist `matched_company` / `company_tier`), **and
the verification payload the funnel pre-fetched for you:**
- **`body_text`** — the **full article body** (up to ~1,500 chars) the funnel extracted
  with a real-browser User-Agent (and a Playwright fallback that also resolves
  `news.google.com` redirect links to the real article). Present only on the top picks the
  funnel enriched; `""` when extraction was blocked/skipped. **When non-empty this is
  Tier-1-grade evidence — equivalent to a successful `WebFetch`, but already on disk.**
- **`extract_status`** — `ok` (body_text is real full text), `blocked` (publisher 403'd
  even the funnel → only the RSS snippet is available), or `skipped` (not in the enriched
  top set).
- **`description`** — the outlet's own RSS snippet (≤300 chars); the Tier-2 fallback when
  `body_text` is empty.
- **`published_raw`** + **`has_timestamp`** — the feed's explicit publish timestamp (bool).
- **`resolved_url`** — the real article URL when the funnel resolved a Google News redirect.

The funnel runs in GitHub Actions, where egress is **open**, so `body_text` is real
first-party evidence **even on a run where *this* routine's own `WebFetch` is 403-blocked**
— that is the WebFetch-403 bypass: prefer the pre-fetched body and you never need to fetch
the article live (see item 4 + Step 1b-ver "Tier 1 — funnel body").
...
1. `Read` the JSON. Parse `candidates[]` into START_POOL. **They are already pre-ranked** — sorted by `score` descending (the funnel's multi-factor relevance score; see the `scoring` block at the top of the JSON, and each item's `score_breakdown`).
2. Apply Gates A + B to each candidate immediately — drop stale (> 24 h), drop URLs already in `RECENT_URLS`.
2b. **Lead selection with the funnel `score`.** Walk the survivors in `score` order and take the top picks from the top of the list — do **not** re-rank from scratch or hunt for your own favourites. `score` already folds in recency, keyword/signal strength, source authority (primary > citation > screening), cross-outlet **corroboration** (`cluster_size`), and — watchlist — the matched company tier (`matched_company` / `company_tier`). Use `cluster_size` as the corroboration count when you note how widely a story was reported. You still apply editorial judgement to **drop** a high-scored item that's off-scope or a near-duplicate of a higher pick, and to ensure topic spread — but the score is the spine of the running order, not a hint.
...
4. If START_POOL has **≥ 8 candidates** after gates → skip WebSearch calls (SEARCH_STRATEGY) and go to Step 1b-ver. Verify each surviving candidate in this priority order — **`body_text` first, before any live fetch**:
   - **Candidate has `body_text` (`extract_status: ok`)** → verify from it as **Tier 1 — funnel body**: the body confirms the headline and (with `published_raw`) the in-WINDOW timestamp; summarize from the body. **No `WebFetch` needed** — this is the bypass, and it works identically whether the probe is `WEBFETCH_OK` or `WEBFETCH_BLOCKED`. Prefer this for every enriched pick.
   - **No `body_text`, probe is `WEBFETCH_OK`** → `WebFetch` the trusted-outlet URL for Tier-1 (body's own timestamp + headline).
   - **No `body_text`, probe is `WEBFETCH_BLOCKED`** → **do NOT fall back to WebSearch** (in a blocked host it is usually unavailable too). Verify at **Tier-2 directly from the funnel**: `source` is an allowlist-matched real publisher, `published_raw` + `has_timestamp` give the in-WINDOW timestamp, and `description` is the citeable snippet. Summarize **only** from `description`; never invent beyond it. Still a real, citeable brief (Tier-2 from funnel), not a degraded stub. (See Step 1b-ver "Tier 2 — funnel snippet".)
...
|---|---|---|
| **Tier 1 — Funnel body** | START_POOL candidate has `body_text` (`extract_status: ok`); the body confirms headline + an in-WINDOW timestamp (`published_raw`); summary drawn from the body | **always** — needs no egress; **prefer this for every enriched pick** |
| **Tier 1 — Full fetch** | `WebFetch` 2xx; body confirms headline + explicit publish date within WINDOW | `WEBFETCH_OK`, and only for picks **without** `body_text` |
| **Tier 2 — Snippet (funnel ▸ or WebSearch)** | domain on trusted-sources; a substantive snippet carrying a timestamp resolvable to within WINDOW; summary paraphrases **only** that snippet. The snippet may come from the **RSS funnel** — a START_POOL candidate's `description` + `published_raw` (**preferred**: first-party, fetched in an unblocked runner, needs no egress from here) — **or** from a live WebSearch result. | when a pick has neither `body_text` nor a usable `WebFetch`; the **sole** non-Tier-1 path when `WEBFETCH_BLOCKED` (use the funnel snippet first) |
| **Drop** | can't satisfy a tier above | — |

**Full-fidelity verification is the target, and the funnel body gets you there for free.**
Per selected story: if it has `body_text`, verify and summarize from that body (Tier 1 —
funnel body) and move on — no fetch. Otherwise, when `WEBFETCH_OK`, find it on a trusted
outlet, `WebFetch` that URL, use the body's own publish timestamp. Only if neither a funnel
body nor a live fetch is available for *this* story, accept Tier 2. Do not pre-emptively
default to Tier 2.

The runner reads the `Verification mode` line in `sources.md` and tags the commit:
`[verify=funnel]` when the run was verified primarily from funnel bodies, `[verify=search]`
when the whole run was forced to Tier-2 snippets (`WEBFETCH_BLOCKED`, no bodies), else
`[verify=webfetch]`.
...
CE, Chinese dates); normalize it. "The body" includes the funnel's `body_text` — read the
timestamp from it just as you would from a live `WebFetch`. When no body is available
(Tier 2), use the funnel candidate's `published_raw` (the feed's own timestamp —
authoritative) over the URL-slug date; fall back to the slug only when neither exists.
...
Verification mode: {funnel | webfetch | search}   # funnel = most picks verified from funnel body_text
Model: {CONFIG.model}
...
1. **{Headline}**
   ...
   - Verification: {Tier 1 — funnel body | Tier 1 — WebFetch | Tier 2 — funnel snippet | Tier 2 — WebSearch snippet}
...
```
Include the ranked-universe Excel link **only when the funnel JSON was used** (Step 0.5
START_POOL available) — that's when `universe-latest.xlsx` reflects this run. On a pure
WebSearch fallback day (no funnel), omit the link.
...
`brief: {TOPIC} {TODAY} [kind={BRIEF_KIND}] [verify={funnel|webfetch|search}]`, pushes, and
